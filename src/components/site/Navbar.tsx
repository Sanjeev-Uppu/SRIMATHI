        ...
        <nav className="hidden lg:flex items-center gap-1 ml-auto">
          {NAV_LINKS.map((l) => (
            <Link
              ...
              activeProps={{ className: "text-[color:var(--royal)]" }}
              inactiveProps={{ className: "text-foreground/80" }}
              className="px-3 py-2 text-sm font-medium hover:text-[color:var(--royal)] relative group"
            >
              {l.label}
              <span className="absolute left-3 right-3 -bottom-0.5 h-[2px] bg-[color:var(--brand)] scale-x-0 group-hover:scale-x-100 group-data-[status=active]:scale-x-100 transition-transform origin-left" />
            </Link>
          ))}
        </nav>
        <div className="hidden lg:flex items-center gap-2 ml-2">
          <a href={`tel:${PHONE}`} className="inline-flex px-4 py-2 rounded-full bg-[color:var(--royal)] text-[color:var(--royal-foreground)] text-sm font-semibold hover:bg-[color:var(--accent)] transition-colors">
            Order Now
          </a>
        </div>
            ...
            <div className="p-4 grid gap-1">
              {NAV_LINKS.map((l) => (
                <Link
                  ...
                  to={l.to}
                  activeOptions={{ exact: true }}
                  activeProps={{ className: "bg-[color:var(--muted)] text-[color:var(--royal)]" }}
                  className="px-3 py-2.5 rounded-lg text-sm font-medium text-foreground hover:bg-[color:var(--muted)]"
                >
                  {l.label}
                </Link>
              ))}
              <a href={`tel:${PHONE}`} className="mt-2 text-center px-4 py-2.5 rounded-lg bg-[color:var(--royal)] text-[color:var(--royal-foreground)] text-sm font-semibold">
                Order Now
              </a>
            </div>
          </motion.div>
